...
#loading data
X=pd.read_csv('D:\\Projects\\BCG_Internship\\Artifacts\\train.csv')
X_test=pd.read_csv('D:\\Projects\\BCG_Internship\\Artifacts\\test.csv')
y=np.ravel(pd.read_csv('D:\\Projects\\BCG_Internship\\Artifacts\\train_labels.csv'))
X_train,X_test,y_train,y_test=train_test_split(X,y,test_size=0.2,random_state=40)
...
for estimator in estimators:
    for max_depth in max_depths:
        ...
        with mlflow.start_run():
            # Log the parameters of the model to MLflow
            mlflow.log_param('n_estimator',estimator)
            ...
            #log the model
            model_name = f'RandomForestClassifier_n_estimator_{estimator}_max_depth_{max_depth}'
            mlflow.sklearn.log_model(RD,model_name)